...
impl std::fmt::Display for LaunchError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::WebConsoleSpawn(e) => write_spawn_error(f, "Web Console", e),
            Self::HealthTimeout { url, waited_secs } => write!(
                f,
                "web console health/UI readiness check timed out after {waited_secs}s at {url}"
            ),
            Self::TauriSpawn(e) => write_spawn_error(f, "Tauri Shell", e),
...
fn write_spawn_error(
    ...
    write!(f, "failed to spawn {role}: {error}")?;
    if error.kind() == std::io::ErrorKind::PermissionDenied
        || matches!(error.raw_os_error(), Some(577 | 1260 | 4551))
    {
        write!(
            f,
            "；Windows 可能因应用控制或签名信任策略阻止了程序。请检查文件的数字签名、SmartScreen、Smart App Control、AppLocker/WDAC 和 Windows 代码完整性事件日志"
        )?;
    }
    Ok(())
}
...
pub fn http_probe(url: &str) -> ProbeOutcome {
    http_probe_with_content_type(url, None)
}

/// 安装态启动探针：API 健康还不够，控制台首页还必须能返回 HTML。
/// 这会把“后端已启动但静态资源缺失”的安装包错误挡在 Tauri 启动之前，
/// 并在 package-selfcheck-last.json 中留下明确的 readiness 失败结果。
pub fn web_console_readiness_probe(health_url: &str) -> ProbeOutcome {
    match http_probe(health_url) {
        ProbeOutcome::Ready => {}
        outcome => return outcome,
    }
    let Some((host, port, _)) = parse_http_url(health_url) else {
        return ProbeOutcome::Transient;
    };
    let ui_url = format!("http://{host}:{port}/");
    http_probe_with_content_type(&ui_url, Some("text/html"))
}

fn http_probe_with_content_type(url: &str, expected_content_type: Option<&str>) -> ProbeOutcome {
    ...
    classify_http_probe_response(head, expected_content_type)
}

fn classify_http_probe_response(
    response_head: &str,
    expected_content_type: Option<&str>,
) -> ProbeOutcome {
    if !response_head.contains(" 200 ") && !response_head.contains(" 200\r") {
        return ProbeOutcome::NotReady;
    }
    let Some(expected) = expected_content_type else {
        return ProbeOutcome::Ready;
    };
    let normalized = response_head.to_ascii_lowercase();
    let expected = expected.to_ascii_lowercase();
    if normalized
        .lines()
        .any(|line| line.starts_with("content-type:") && line.contains(expected.as_str()))
    {
        ProbeOutcome::Ready
    } else {
...
mod tests {
    ...
    #[test]
    fn permission_denied_spawn_error_explains_windows_signature_policy() {
        let error = LaunchError::WebConsoleSpawn(std::io::Error::from(
            std::io::ErrorKind::PermissionDenied,
        ));
        let message = error.to_string();
        assert!(message.contains("签名信任策略"));
        assert!(message.contains("AppLocker/WDAC"));
    }
    ...
    #[test]
    fn ui_probe_requires_successful_html_response() {
        let html = "HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=utf-8\r\n\r\n";
        let json = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n";
        let missing = "HTTP/1.1 404 Not Found\r\nContent-Type: text/plain\r\n\r\n";

        assert_eq!(
            classify_http_probe_response(html, Some("text/html")),
            ProbeOutcome::Ready
        );
        assert_eq!(
            classify_http_probe_response(json, Some("text/html")),
            ProbeOutcome::NotReady
        );
        assert_eq!(
            classify_http_probe_response(missing, Some("text/html")),
            ProbeOutcome::NotReady
        );
    }
