...
$frontendResources = @(
    @('gui-web.frontend-index', 'modules/gui-web/packages/web-console/index.html'),
    @('gui-web.frontend-app', 'modules/gui-web/packages/web-console/src/app.js'),
    @('gui-web.frontend-styles', 'modules/gui-web/packages/web-console/src/styles.css'),
    @('gui-web.frontend-realtime-capture', 'modules/gui-web/packages/web-console/src/realtime_voice_capture.js'),
    @('gui-web.frontend-realtime-output', 'modules/gui-web/packages/web-console/src/realtime_audio_output.js'),
    @('gui-web.frontend-stt-tail', 'modules/gui-web/packages/web-console/src/stt_tail_capture.js')
)
foreach ($expected in $frontendResources) {
    $id = [string]$expected[0]
    $path = [string]$expected[1]
    if ($resourceIds -notcontains $id) {
        throw "package manifest must include $id resource"
    }
    $resource = @($manifest.resources | Where-Object { $_.id -eq $id })[0]
    if ([string]$resource.source -ne $path) {
        throw "unexpected $id source: $($resource.source)"
    }
    if ([string]$resource.target -ne $path) {
        throw "unexpected $id target: $($resource.target)"
    }
}

if ($resourceIds -notcontains 'gui-web.assets') {
    throw 'package manifest must include gui-web.assets resource'
}
$webAssets = @($manifest.resources | Where-Object { $_.id -eq 'gui-web.assets' })[0]
if ([string]$webAssets.target -ne 'modules/gui-web/packages/web-console/assets') {
    throw "unexpected gui-web.assets target: $($webAssets.target)"
}
if ($webAssets.optional -eq $true) {
    throw 'gui-web.assets must be required for packaged console rendering'
}
