...
mod tests {
  ...
  "commands": [
    ...
    fn write_lifecycle_plugin(root: &Path, name: &str, version: &str) -> PathBuf {
        ...
        write_file(
            root.join("lifecycle")
                ...
                .as_path(),
            lifecycle_script("init"),
        );
        write_file(
            root.join("lifecycle")
                .join(format!("shutdown.{extension}"))
                .as_path(),
            lifecycle_script("shutdown"),
        );
        write_file(
            root.join(MANIFEST_RELATIVE_PATH).as_path(),
            format!(
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(&script_path, tool_echo_json_script());
        #[cfg(unix)]
        {
    ...
    #[cfg(windows)]
    fn lifecycle_script(phase: &str) -> &'static str {
        match phase {
    ...
    #[cfg(windows)]
    fn tool_echo_json_script() -> &'static str {
        "@echo off\r\nset /p INPUT=\r\necho {\"plugin\":\"%CLAW_PLUGIN_ID%\",\"tool\":\"%CLAW_TOOL_NAME%\",\"input\":%INPUT%}\r\n"
    }
  ...
  "commands": [
    ...
    #[test]
    fn default_bundled_root_exposes_cli_anything_bridge_tools() {
        let config_home = temp_dir("default-bundled-cli-anything-home");
        let manager = PluginManager::new(PluginManagerConfig::new(&config_home));
        ...
        let installed = manager
            ...
            .expect("default bundled plugins should auto-install");
        assert!(installed.iter().any(|plugin| {
            plugin.metadata.id == "cli-anything-bridge@bundled" && plugin.enabled
        }));

        let tools = manager
            .aggregated_tools()
            .expect("bundled tools should aggregate");
        assert!(tools
            .iter()
            .any(|tool| tool.definition().name == "cli_anything_status"));
        assert!(tools
            .iter()
            .any(|tool| tool.definition().name == "cli_anything_run"));

        let status_tool = tools
            .iter()
            .find(|tool| tool.definition().name == "cli_anything_status")
            .expect("cli_anything_status tool should exist");
        let output = status_tool
            .execute(&serde_json::json!({}))
            .expect("cli_anything_status should execute");
        let payload: Value = serde_json::from_str(&output).expect("status output should be json");
  ...
  "description": "Registry fallback plugin"
    ...
    fn aggregates_and_executes_plugin_tools() {
        ...
        let tool = tools
            .iter()
            .find(|tool| tool.plugin_id() == "tool-demo@external")
            .expect("installed plugin tool should aggregate");
        assert_eq!(tool.definition().name, "plugin_echo");
        assert_eq!(tool.required_permission(), "workspace-write");
