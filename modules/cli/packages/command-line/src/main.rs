...
use runtime::{
    ...
    PermissionGateReport, PermissionMode, PermissionPolicy, ProjectContext, RuntimeError,
    RuntimeToolContext, Session, SessionGrantView, TokenUsage, ToolCaller, ToolError, ToolExecutor,
    ToolInvocationExecutor, ToolInvoke, ToolOutcome, ToolOutcomeStatus, UsageTracker,
};
use serde_json::json;
...
fn load_config_to_env() -> Result<Option<String>, Box<dyn std::error::Error>> {
    ...
    if let Ok(config) = loader.load() {
        if let Some(api_base_url) = config.get("apiBaseUrl").and_then(|v| v.as_str()) {
            env::set_var("OPENAI_BASE_URL", api_base_url);
    ...
    fn startup_banner(&self) -> String {
        ...
        let mut lines = vec![
            ...
            format!(
                ...
                status
                    .as_ref()
                    .map_or_else(active_profile_name, |context| context.profile.clone())
            ),
            format!(
                "  Context engine   {}{}",
                status
                    .as_ref()
                    .map_or(ContextEngineMode::Full, |context| context.context_engine)
                    .as_str(),
                if status.as_ref().is_some_and(|context| context.fast_mode) {
                    " + fast"
                } else {
                    ""
                }
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let mut path = cwd.join(".claw").join("sessions");
...
fn status_context(
    ...
    Ok(StatusContext {
        ...
        project_root,
        git_branch,
        profile: runtime_config.agent().profile().to_string(),
        context_engine: runtime_config.agent().context_engine(),
        fast_mode: runtime_config.agent().fast_mode(),
    })
}
  ...
  Merged keys       {}",
    ...
    if let Some(section) = section {
        ...
        let value = match section {
            ...
            "agent" => runtime_config.get("agent"),
            "plugins" => runtime_config
                .get("plugins")
                .or_else(|| runtime_config.get("enabledPlugins")),
            other => {
                lines.push(format!(
                    "  Unsupported config section '{other}'. Use env, hooks, model, agent, or plugins."
                ));
                return Ok(lines.join(
                    "
",
...
fn build_system_prompt(model: Option<&str>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
...
fn build_runtime_plugin_state(
    ...
    let runtime_config = loader.load()?;
    let plugin_manager = build_plugin_manager(&cwd, &loader, &runtime_config);
    let feature_config = merge_feature_config_with_plugin_hooks(
...
fn merge_feature_config_with_plugin_hooks(
    feature_config: runtime::RuntimeFeatureConfig,
    plugin_hooks: &PluginHooks,
) -> runtime::RuntimeFeatureConfig {
    if plugin_hooks.pre_tool_use.is_empty() && plugin_hooks.post_tool_use.is_empty() {
        return feature_config;
    }

    let merged_hooks = feature_config
        .hooks()
        .merged(&runtime::RuntimeHookConfig::new(
...
struct CliRegistryInvocationExecutor {
    tool_registry: GlobalToolRegistry,
}

impl ToolInvocationExecutor for CliRegistryInvocationExecutor {
    ...
    fn execute(&self, invoke: &ToolInvoke) -> ToolOutcome {
        ...
        match self.tool_registry.execute(&invoke.tool_name, &invoke.input) {
            Ok(output) => ToolOutcome {
                ...
                permission_gate: PermissionGateReport::allow_auto(
                    PermissionMode::ReadOnly,
                    "cli-executor-placeholder",
                ),
                evidence: None,
            },
...
impl ToolExecutor for CliToolExecutor {
    fn execute(&mut self, tool_name: &str, input: &str) -> Result<String, ToolError> {
        ...
        let outcome = execute_cli_tool_through_runtime(
            &self.tool_registry,
            self.permission_mode,
            tool_name,
            value,
        ...
        let output = outcome
            ...
            .map_or_else(|| outcome.summary_text.clone(), ToString::to_string);
        let is_error = outcome.is_error() || outcome.status == ToolOutcomeStatus::DryRunOnly;
        if self.emit_output {
        ...
        if is_error {
            Err(ToolError::new(outcome.summary_text))
        } else {
            Ok(output)
        }
    }
...
fn execute_cli_tool_through_runtime(
    tool_registry: &GlobalToolRegistry,
    permission_mode: PermissionMode,
    tool_name: &str,
    ...
    let workspace_root = env::current_dir()
        .map_err(|error| ToolError::new(format!("cannot resolve current workspace: {error}")))?;
    let path_targets = tools::path_effect::extractor_for(tool_name).extract(&input);
    let protected_rules = default_protected_rules();
    let (session_authorized, session_confirmed_twice) = cli_runtime_grant(permission_mode);
    let runtime_executor = CliRegistryInvocationExecutor {
        tool_registry: tool_registry.clone(),
    };
    let executors: [&dyn ToolInvocationExecutor; 1] = [&runtime_executor];
    let profile = if permission_mode == PermissionMode::DangerFullAccess {
        runtime::PermissionProfile::FullAccess
    } else {
        runtime::PermissionProfile::WorkspaceAuto
    };
    let invoke = ToolInvoke {
        call_id: format!("cli-tool-{}", generate_session_id()),
        tool_name: tool_name.to_string(),
        input,
        caller: ToolCaller::Cli,
        workspace_id: workspace_root.to_string_lossy().to_string(),
        session_id: None,
        user_authorized: session_authorized,
        user_confirmed_twice: session_confirmed_twice,
    };
    let ctx = RuntimeToolContext {
        workspace_root: &workspace_root,
        required_permission: required_permission_for_cli_tool(tool_registry, tool_name),
        path_targets,
    ...
    Ok(runtime_tool_execute(invoke, &ctx))
}

fn required_permission_for_cli_tool(
    tool_registry: &GlobalToolRegistry,
    tool_name: &str,
) -> PermissionMode {
    tool_registry
...
mod tests {
    ...
    #[test]
    fn cli_tool_executor_routes_read_file_through_runtime() {
        let path = std::env::temp_dir().join(format!(
            "coolzhu-cli-runtime-{}.txt",
            super::generate_session_id()
        ));
        fs::write(&path, "runtime cli bridge").expect("write temp file");
        let mut exec = CliToolExecutor::new(
            None,
            false,
        ...
        assert!(output.contains("runtime cli bridge"));
        let _ = fs::remove_file(path);
    }

    #[test]
    fn cli_tool_executor_uses_runtime_gate_for_workspace_write_outside() {
        let outside = std::env::temp_dir().join(format!(
            "coolzhu-cli-runtime-outside-{}.txt",
            super::generate_session_id()
        ));
        let mut exec = CliToolExecutor::new(
            None,
        ...
        let error = exec
            .execute(
                "write_file",
                &json!({ "path": outside, "content": "blocked" }).to_string(),
            )
            .expect_err("workspace-write outside workspace should require approval");

        assert!(error.to_string().contains("awaiting user approval"));
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("/config [env|hooks|model|agent|plugins]"));
    ...
    fn status_line_reports_model_and_token_totals() {
        let status = format_status_report(
            ...
            &super::StatusContext {
                ...
                profile: "daily-zhipu".to_string(),
                context_engine: ContextEngineMode::Focused,
                fast_mode: true,
            },
        );
        ...
        assert!(status.contains("Profile          daily-zhipu"));
        assert!(status.contains("Context engine   focused"));
        assert!(status.contains("Fast mode        enabled"));
