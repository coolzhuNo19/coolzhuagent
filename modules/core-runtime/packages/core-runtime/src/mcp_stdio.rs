...
use crate::permission_gate::{PathTarget, ProtectedRule, SessionGrantView};
use crate::permissions::{PermissionMode, PermissionProfile};
use crate::tool::{
    runtime_tool_execute, PermissionGateReport, RuntimeToolContext, ToolCaller,
    ToolInvocationExecutor, ToolInvoke, ToolOutcome, ToolOutcomeStatus,
};
...
struct McpRuntimeApprovalExecutor<'a> {
    tool_name: &'a str,
}

impl ToolInvocationExecutor for McpRuntimeApprovalExecutor<'_> {
    ...
    fn execute(&self, invoke: &ToolInvoke) -> ToolOutcome {
        ToolOutcome {
            ...
            output: json!({"route": "mcp-runtime-approved"}),
            summary_text: "mcp runtime permission approved".to_string(),
            elapsed_ms: 0,
            permission_gate: PermissionGateReport::allow_auto(
                PermissionMode::ReadOnly,
                "mcp-runtime-placeholder",
            ),
            evidence: None,
        }
    }
...
impl McpServerManager {
    ...
    /// Execute a discovered MCP tool through the unified runtime permission gate.
    ///
    /// The low-level `call_tool` method is kept for protocol tests and direct MCP
    /// client use. Product call sites should prefer this wrapper so MCP-originated
    /// tool invocations get the same `ToolInvoke(caller=Mcp)` audit and approval
    ...
    pub async fn call_tool_through_runtime(
        ...
        let executors: [&dyn ToolInvocationExecutor; 1] = [&approval_executor];
        let runtime_ctx = RuntimeToolContext {
            workspace_root,
            required_permission,
            path_targets,
            protected_rules,
        ...
        let gate = gate_outcome.permission_gate;
        let started = Instant::now();
        let arguments = match invoke.input.clone() {
            JsonValue::Null => None,
            value => Some(value),
        };
        match self.call_tool(&invoke.tool_name, arguments).await {
            Ok(response) => mcp_response_to_tool_outcome(
                &invoke,
                response,
                gate,
                started.elapsed().as_millis() as u64,
            ),
            Err(error) => mcp_manager_error_to_tool_outcome(
                &invoke,
                error,
                gate,
                started.elapsed().as_millis() as u64,
...
fn mcp_response_to_tool_outcome(
    invoke: &ToolInvoke,
    response: JsonRpcResponse<McpToolCallResult>,
    permission_gate: PermissionGateReport,
    elapsed_ms: u64,
) -> ToolOutcome {
    if let Some(error) = response.error {
        return ToolOutcome {
            ...
            summary_text: format!("MCP tool '{}' failed: {}", invoke.tool_name, error.message),
            elapsed_ms,
            permission_gate,
            evidence: None,
        };
    ...
    let Some(result) = response.result else {
        return ToolOutcome {
            ...
            summary_text: format!("MCP tool '{}' returned no result", invoke.tool_name),
            elapsed_ms,
            permission_gate,
            evidence: None,
        };
    ...
    let summary_text = mcp_tool_result_summary(&invoke.tool_name, &result, is_error);
...
fn mcp_manager_error_to_tool_outcome(
    invoke: &ToolInvoke,
    ...
    ToolOutcome {
        ...
        summary_text: format!("MCP tool '{}' failed: {}", invoke.tool_name, message),
        elapsed_ms,
        permission_gate,
        evidence: None,
    }
...
fn mcp_tool_result_summary(tool_name: &str, result: &McpToolCallResult, is_error: bool) -> String {
    let status = if is_error { "failed" } else { "completed" };
    if let Some(text) = result.content.iter().find_map(|content| {
        if content.kind == "text" {
            content.data.get("text").and_then(JsonValue::as_str)
        } else {
            None
        }
    }) {
        let preview = text.chars().take(320).collect::<String>();
        return format!("MCP tool '{tool_name}' {status}: {preview}");
    }
    if result.structured_content.is_some() {
        return format!("MCP tool '{tool_name}' {status} with structured content");
    }
    format!("MCP tool '{tool_name}' {status}")
}
...
mod tests {
    ...
    use crate::permission_gate::{
        default_protected_rules, PathAccess, PathTarget, SessionGrantView,
    };
    use crate::permissions::PermissionMode;
    use crate::tool::{PermissionDecision, ToolCaller, ToolInvoke, ToolOutcomeStatus};
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("runtime-mcp-stdio-{pid}-{nanos}-{unique}"))
    }

    fn make_script_executable(_path: &Path) {
        #[cfg(unix)]
        {
            let mut permissions = fs::metadata(_path).expect("metadata").permissions();
            permissions.set_mode(0o755);
            fs::set_permissions(_path, permissions).expect("chmod");
        }
    }
    ...
    fn write_echo_script() -> PathBuf {
        ...
        let script = [
            ...
            "if line:",
            "    sys.stdout.write(f\"ECHO:{line.rstrip(chr(10))}\\n\")",
            "    sys.stdout.flush()",
            "",
        ]
        .join("\n");
    ...
    fn spawns_stdio_process_and_round_trips_io() {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(trim_line_end(&ready), "READY:secret-value");
            ...
            assert_eq!(trim_line_end(&echoed), "ECHO:ping from client");
    ...
    fn direct_spawn_uses_transport_env() {
        ...
        runtime.block_on(async {
            ...
            assert_eq!(
                trim_line_end(&String::from_utf8_lossy(&ready)),
                "READY:direct-secret"
            );
    ...
    #[test]
    fn manager_call_tool_through_runtime_runs_allowed_mcp_tool() {
        let runtime = Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("runtime");
        runtime.block_on(async {
            ...
            let outcome = manager
                .call_tool_through_runtime(
                    ToolInvoke {
                        call_id: "mcp-call-1".to_string(),
                        tool_name: mcp_tool_name("alpha", "echo"),
                        input: json!({"text": "runtime"}),
                        caller: ToolCaller::Mcp,
                        workspace_id: "ws-mcp".to_string(),
                        session_id: Some("session-mcp".to_string()),
    ...
    #[test]
    fn manager_call_tool_through_runtime_blocks_mcp_workspace_write_before_call() {
        let runtime = Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("runtime");
        runtime.block_on(async {
            ...
            let outcome = manager
                .call_tool_through_runtime(
                    ...
                    vec![PathTarget::new(outside, PathAccess::Write)],
                    &rules,
                    SessionGrantView::default(),
                )
                .await;

            assert_eq!(outcome.status, ToolOutcomeStatus::DryRunOnly);
            assert_eq!(
                outcome.permission_gate.decision,
                PermissionDecision::RequireApproval
            );
            assert!(
                outcome.summary_text.contains("(mcp)"),
                "pending summary should identify the MCP caller"
            );
            let log = fs::read_to_string(&log_path).expect("read log");
            assert_eq!(
                log.lines().collect::<Vec<_>>(),
