...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let feature_config = RuntimeFeatureConfig {
            ...
            agent: parse_optional_agent_config(&merged_value)?,
            mcp: McpConfigCollection {
                servers: mcp_servers,
            },
...
impl RuntimeConfig {
    ...
    #[must_use]
    pub fn agent(&self) -> &RuntimeAgentConfig {
        &self.feature_config.agent
    }
...
impl RuntimeFeatureConfig {
    ...
    #[must_use]
    pub fn with_agent(mut self, agent: RuntimeAgentConfig) -> Self {
        self.agent = agent;
        self
    }
...
pub fn default_config_home() -> PathBuf {
    ...
    let base = default_home_dir()
        .map(|home| home.join(".claw"))
        .unwrap_or_else(|| PathBuf::from(".claw"));
    profile_scoped_path(base)
}

#[must_use]
pub fn active_profile_name() -> String {
    profile_override_from_env().unwrap_or_else(|| "default".to_string())
}
...
fn profile_scoped_path(base: PathBuf) -> PathBuf {
    match profile_override_from_env().as_deref() {
        Some("default") | None => base,
        Some(profile) => base.join("profiles").join(profile),
    }
}

fn profile_override_from_env() -> Option<String> {
    std::env::var("CLAW_PROFILE").ok().and_then(|value| {
        let trimmed = value.trim();
        (!trimmed.is_empty()).then(|| trimmed.to_string())
    })
}

fn context_engine_override_from_env() -> Result<Option<ContextEngineMode>, ConfigError> {
    std::env::var("CLAW_CONTEXT_ENGINE")
        .ok()
        .map(|value| parse_context_engine_label(value.trim(), "CLAW_CONTEXT_ENGINE"))
        .transpose()
}

fn fast_mode_override_from_env() -> Result<Option<bool>, ConfigError> {
    let Some(value) = std::env::var("CLAW_FAST_MODE").ok() else {
        return Ok(None);
    };
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        other => Err(ConfigError::Parse(format!(
            "CLAW_FAST_MODE: unsupported boolean value {other}"
        ))),
    }
}

impl Default for RuntimeAgentConfig {
    fn default() -> Self {
        Self {
            profile: active_profile_name(),
            context_engine: ContextEngineMode::default(),
            fast_mode: false,
        }
    }
}
...
impl RuntimeAgentConfig {
    ...
    #[must_use]
    pub const fn context_engine(&self) -> ContextEngineMode {
        self.context_engine
    }
...
fn parse_optional_agent_config(root: &JsonValue) -> Result<RuntimeAgentConfig, ConfigError> {
    let mut config = RuntimeAgentConfig::default();
    if let Some(agent) = root
        .as_object()
        .and_then(|object| object.get("agent"))
        .map(|value| expect_object(value, "merged settings.agent"))
        .transpose()?
    {
        if let Some(profile) = optional_string(agent, "profile", "merged settings.agent")? {
            config.profile = profile.to_string();
        }
        if let Some(context_engine) =
            optional_string(agent, "contextEngine", "merged settings.agent")?
        {
            config.context_engine =
                parse_context_engine_label(context_engine, "merged settings.agent.contextEngine")?;
        }
        if let Some(fast_mode) = optional_bool(agent, "fastMode", "merged settings.agent")? {
            config.fast_mode = fast_mode;
        }
    }

    if let Some(profile) = profile_override_from_env() {
        config.profile = profile;
    }
    if let Some(context_engine) = context_engine_override_from_env()? {
        config.context_engine = context_engine;
    }
    if let Some(fast_mode) = fast_mode_override_from_env()? {
        config.fast_mode = fast_mode;
    }
...
fn parse_context_engine_label(
    value: &str,
    context: &str,
) -> Result<ContextEngineMode, ConfigError> {
    match value {
        "full" => Ok(ContextEngineMode::Full),
        "focused" => Ok(ContextEngineMode::Focused),
        "minimal" => Ok(ContextEngineMode::Minimal),
        other => Err(ConfigError::Parse(format!(
            "{context}: unsupported context engine {other}"
        ))),
    }
}
...
mod tests {
    use super::{
        ConfigLoader, ConfigSource, ContextEngineMode, McpServerConfig, McpTransport,
    ...
    fn parses_agent_runtime_config() {
        ...
        assert_eq!(loaded.agent().profile(), "daily-zhipu");
        assert_eq!(loaded.agent().context_engine(), ContextEngineMode::Focused);
