...
| REQ-WEB-WIN-002 | 设置独立窗口 | UI/API | P0 | 测试中 | 会话/模型/Custom provider/视觉 Agent/TTS-STT/工具目录分区清晰；工具与审批栏默认按设计图显示本地模型、CLI、MCP、Skill、compute-use、Semantic Dispatch 六行状态清单，旧详情仅在“管理/启用/计划配置”后按需展开；本地模型入口显示“本地文本推理”，支持 `.gguf` 模型文件路径选择与保存；工具调用状态以红/绿/灰状态灯表达，聊天流不展示工具调用细节 | 前端窗口首版已接现有会话、工具、音频和视觉 Agent 配置；2026-05-18 工具区补齐 `/api/tools/{tool_id}` 详情、`/api/tools/dispatch` 语义派发 dry-run 和场景/权限矩阵；2026-06-20 工具与审批栏完成设计图对齐，保留真实目录与语义派发能力但默认折叠详细控制区，Computer Use 已验证管理面板展开/收起与计划配置切换；2026-06-26 本地模型工具行补 `.gguf` 文件路径输入、Windows 文件选择器、路径校验和保存 API |
...
| REQ-WEB-WIN-008 | 诊断日志独立窗口 | UI/API | P1 | 测试中 | 显示 diagnostics health、修复建议、日志 tail、工具审计日志筛选；只记录关键步骤诊断，不提供调试拦截/禁用执行类开关；Web 绑定端口诊断必须与实际启动变量 `COOLZHU_WEB_BIND_ADDR` 一致；Web Console 异常退出不得由本地模型子进程遗留 8765 listener | 前端首版完成：health、修复建议、工具审计摘要、logs tail 占位；2026-05-20 调整：移除 `goal.runtime` 禁用派发口径，Goal 运行关键步骤改通过 `goal_events` caller/payload 和诊断日志定位；同日补 Web bind 诊断变量一致性。2026-06-25 增加 listener non-inherit、Job Object 子进程回收和 launcher 安全兜底；真实强杀 Web Console 后 Gemma 自动退出、8765 释放并可立即重启 |
...
| REQ-LLM-005 | LLM Adapter | Custom 自定义厂商 / OpenAI-compatible 模型配置 | Provider | P1 | 开发中 | Provider 列表新增 `Custom`，既可接本地模型，也可接不在配置总表内的厂商模型和中转平台；三合一卡片保持现有紧凑布局，`base_url` 和 `endpoint` 仅在 provider=Custom 时显示与保存；本地模型可见身份统一为 `coolzhu-model`，回复与推理卡片不得暴露原模型名或 GGUF 文件名；reasoning effort 低/中/高/超高/最大继续可用 | 2026-06-18 审计发现 Custom 空 key 会发送 `Bearer EMPTY`、历史非 Custom base_url 可能误走 OpenAI-compatible 等协议漂移，回到开发中按协议矩阵修复；2026-06-26 增加本地模型可见品牌约束与回复/推理文本脱敏 |
...
| REQ-GOAL-005 | Skill 注册与执行约束 | Agent 协同 | P2 | 开发中 | 提供 plan、implement、test、review、simplify、debug、document、design、security-audit、release、verify、decompose、estimate、benchmark 14 个 goal skill；所有 Goal phase 执行默认加载项目 baseline Skills：会话链路、模型推理、工具执行；每个 skill 声明 requires_tools/requires_permission | 与 `REQ-TOOL-012` 适配矩阵对齐；2026-06-26 新增 `coolzhu-goal-session-chain`、`coolzhu-goal-model-reasoning`、`coolzhu-goal-tool-execution` 三个项目内置 Skill，并在 Goal prompt 注入正文 |
...
| REQ-WEB-LOCAL-MODEL-001 | GUI Web / Vision | 设置窗口本地模型服务切换 | 本地模型 | P0 | 测试中 | 本地文本推理、UI-DETR/ShowUI、全部关闭按钮能进入后端；启动中、ready、degraded、失败均可见；错误不得静默；本地会话预算必须服从服务实际上下文；本地文本推理模型文件由 `coolzhu.toml [model].local_chat_model_path` 管理，前端可选择 `.gguf` 文件并保存 | 2026-06-24 已修复 45,682 tokens 超出 llama-server `n_ctx=8192`：本地预算强制服从 config，输出限制 2048，小上下文禁用工具，否定式搜索不再误注入工具；真实聊天室返回 `LOCAL_CHAIN_OK`，待用户人工确认前端视觉与切换交互；2026-06-26 移除硬编码 Gemma 路径，补模型文件路径配置 API 与前端输入栏，用户可见文案改为“本地文本推理”/`coolzhu-model` |
...
| 日期 | 变更 | 内容 |
| --- | --- | --- |
| 2026-06-26 | 修复 | 修复聊天消息默认头像残留马里奥：普通用户消息默认走武侠头像资源，初始渲染和历史渲染统一。设置窗口本地模型工具行改为“本地文本推理”，新增 `.gguf` 模型路径输入、Windows 文件选择器、路径校验与保存 API；启动链路移除硬编码 Gemma 路径，状态与可见回复/推理统一显示 `coolzhu-model` 并脱敏原模型名。将 GLM5.2 Goal 长任务复盘问题拆成 `coolzhu-goal-session-chain`、`coolzhu-goal-model-reasoning`、`coolzhu-goal-tool-execution` 三个项目 Skill，Goal prompt 默认加载 baseline skill 正文。自动化功能验证通过，前端视觉/交互由用户人工确认。 |
...
| 2026-06-25 | 修复 | `REQ-WEB-WIN-008` 8765 幽灵监听：Web Console bind 后清除 socket 继承标志，本地模型进程加入 `KILL_ON_JOB_CLOSE` Job Object；launcher 根据配置端口识别死亡 owner/已知 holder，仅安全清理遗留模型进程，对 live Web Console 或未知进程快速失败且不误杀；真实切换 Gemma 后仅强杀 Web Console，`llama-server.exe` 自动退出且 8765 立即释放，package 重启 health HTTP 200。 |
...
| 2026-06-25 | 修复 | 方案 A 完成本地 Gemma 4 12B 回复完整性与 Agnes 图生视频协议修复：确认模型架构上限为 256K，本机运行档调整为 16K context / 4K output / 1K reasoning / 单槽；提示词硬预算纳入图片 token 与 1024 token 安全余量，本地会话达到阈值复用统一自动压缩和记忆回灌；超预算优先截断文本并保留当前图片；Agnes 顶层 `image` 在 Provider 边界由 Data URI 转为校验后的裸 Base64，并持久化异步失败消息。550 项串行测试、package all、HTTP 健康检查和真实本地模型正文返回通过；前端视觉与真实计费视频生成由用户人工确认 |
